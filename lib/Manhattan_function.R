manhattan <- function(x, chr="CHR", bp="BP", p="P", snp="SNP", 
                      col=c('#29788E', '#22A784', '#79D151'), chrlabs=NULL,
  ...
  if (suggestiveline) abline(h=suggestiveline, col="blue", lty=1, lwd=3)
  if (genomewideline) abline(h=genomewideline, col="red", lty=2, lwd=3)
    ...
    if (annotateTop == FALSE) {
      if (logp) {
        with(subset(d, P <= annotatePval), 
             text(pos, -log10(P), font=2,pos=4, labels = topHits$SNP, cex = 1), ...)
      } else
        with(subset(d, P >= annotatePval), 
             text(pos, P, font=2,pos=4, labels = topHits$SNP, cex = 1), ...)
    }
    else {
      ...
      if (logp ){
        textxy(topSNPs$pos, -log10(topSNPs$P), font=2,pos=4, labels = topSNPs$SNP, cex = 1, ...)
      } else
        textxy(topSNPs$pos, topSNPs$P, font=2,pos=4, labels = topSNPs$SNP, cex = 1, ...)
    }
  }  
  par(xpd = FALSE,mar = c(5.4,5,4,4))
}
